...
class DatasetLoader():
    ...
    def load(self,type_):
        if type_ in ["parallel", "parallel-align"]:
            self._download(self._parallel_data)
            parallel_source, parallel_target = list(), list()
            index = 0 if isfile(join(self._parallel_data["path"], self._parallel_data["filenames"][0])) else 1
            with gopen(join(self._parallel_data["path"], self._parallel_data["filenames"][index]), 'rt') as tsvfile:
                samples = self._parallel_data["samples"][1 if type_.endswith("align") else 0]
            ...
            self._download(self._monolingual_data)
            mono_source, mono_target= list(), list()
            mpath, mfiles = self._monolingual_data["path"], self._monolingual_data["filenames"]
            with gopen(join(mpath, mfiles[0]), "rt") as f, gopen(join(mpath, mfiles[1]), "rt") as g, \
                ...
                for src in f:
                    if self.min_monolingual_sent_len <= len(src_tokenize(src)) <= self.max_monolingual_sent_len:
                        ...
                        if collected_src_samples >= self._monolingual_data["samples"][1 if type_.endswith("train") else 0]:
                            break
                for tgt in g:
                    if self.min_monolingual_sent_len <= len(tgt_tokenize(tgt)) < self.max_monolingual_sent_len:
                        ...
                        if collected_tgt_samples >= self._monolingual_data["samples"][1 if type_.endswith("train") else 0]:
                            break
            ...
            if type_.endswith("mlqe"):
                self._download(self._mlqe_eval_data)
                samples, member = self._mlqe_eval_data["samples"], self._mlqe_eval_data["member"]
                with topen(join(self._mlqe_eval_data["path"], self._mlqe_eval_data["filename"]), 'r:gz') as tf:
                ...
                self._download(self._wmt_eval_data)
                with open(join(self._wmt_eval_data["path"], self._wmt_eval_data["filename"]), newline='') as f:
                    ...
                    MosesDetokenizer(self.target_lang) as tgt_detokenize:
                        for _, src, mt, _, score, _ in islice(tsvdata, 1, self._wmt_eval_data["samples"] + 1):
                            eval_source.append(src_detokenize(src.split()))
                            eval_system.append(get_true_case(tgt_detokenize(mt.split())))
                            eval_scores.append(float(score))
            else:
                self._download(self._news_eval_data)
                samples, members = self._news_eval_data["samples"], self._news_eval_data["members"]
                with topen(join(self._news_eval_data["path"], self._news_eval_data["filename"]), 'r:gz') as tf:
