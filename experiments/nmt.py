...
def nmt_tests(metric="cosine", weights=[0.8, 0.2], max_len=30, nmt_iterations=0):
    ...
    results, index = defaultdict(list), list(range(remap_iterations + 1)) +[f"{remap_iterations} + NMT-{iteration}"
            for iteration in range(nmt_iterations + 1)]
...
for weight in linspace(1, 0, 11):
    print(f"Using weight {weight} for cross-lingual XMoverScore and weight {1 - weight} for NMT system.")
    print(*nmt_tests(metric="wmd", weights=[weight, 1 - weight], iterations=3), sep="\n")
    print(*nmt_tests(metric="wmd", weights=[weight, 1 - weight], iterations=3, max_len=50), sep="\n")
