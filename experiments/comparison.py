...
from numpy import argsort, corrcoef
...
from metrics.xmoverscore import XMoverNMTLMBertAlignScore, XMoverScore
...
def transquest_tests(source_lang, target_lang, dataset_name):
    ...
    model = MonoTransQuestModel(
        "xlmroberta", model_name, num_labels=1, use_cuda=cuda_is_available()
    )
    dataset = DatasetLoader(source_lang, target_lang)
    eval_src, eval_system, eval_scores = dataset.load(dataset_name)
...
def xmoverscore_tests(source_lang, target_lang, dataset_name, mapping="UMD"):
    scorer = XMoverScore(
        mapping=mapping, use_lm=True, lm_model_name=lm_model[target_lang]
    )
    dataset = DatasetLoader(source_lang, target_lang)
    eval_src, eval_system, eval_scores = dataset.load(dataset_name)
...
def distilscore_tests(source_lang, target_lang, dataset_name):
    scorer = DistilScore(
        student_model_name="xlm-r-bert-base-nli-stsb-mean-tokens",
        source_language=source_lang,
        target_language=target_lang,
        student_is_pretrained=True,
        suffix="1",
    )
    dataset = DatasetLoader(source_lang, target_lang)
    eval_src, eval_system, eval_scores = dataset.load(dataset_name)
...
def uscore_tests(source_lang, target_lang, dataset_name, max_len=30):
    xmover = XMoverNMTLMBertAlignScore(
        src_lang=source_lang,
        tgt_lang=target_lang,
        lm_weights=[1, 0.1],
        nmt_weights=[0.5, 0.4],
        use_lm=True,
        lm_model_name=lm_model[target_lang],
    )
    contrast = ContrastScore(
        source_language=source_lang, target_language=target_lang, parallelize=True
    )
    ...
    results, index = defaultdict(list), [
        f"UScore (WRD) ({max_len} tokens)",
        f"UScore (SNT) ({max_len} tokens)",
        f"UScore (WRD) + UScore (SNT) ({max_len} tokens)",
    ]

    logging.info("Evaluating UScore (WRD)")
    for iteration in range(1, remap_iterations + 1):
        logging.info(f"Remapping iteration {iteration}.")
        xmover.remap(
            mono_src, mono_tgt, suffix=suffix + f"-{iteration}", overwrite=False
        )
    for iteration in range(nmt_iterations):
        ...
        xmover.train(
            train_src,
            train_tgt,
            suffix=suffix + f"-{remap_iterations}",
            iteration=iteration,
            overwrite=False,
            k=1,
        )
    ...
    logging.info("Evaluating UScore (SNT)")
    ...
    logging.info("Evaluating UScore (WRD) + UScore (SNT)")
    wmd_scores, contrast_scores = xmover.score(eval_src, eval_system), contrast.score(
        eval_src, eval_system
    )
    pearson, spearman = correlation(
        [0.6 * x + 0.4 * y for x, y in zip(wmd_scores, contrast_scores)], eval_scores
    )
    logging.info(f"Pearson: {pearson}, Spearman: {spearman}")
    results["pearson"].append(round(100 * pearson, 2))
    results["spearman"].append(round(100 * spearman, 2))
